...
if [ ! -d "$DIR_PROGRAMAS" ]; then
    echo "directory not found: $DIR_PROGRAMAS"
    exit 1
fi

# Find the latest directory inside $DIR_PROGRAMAS using ls and sort
latestDirectory=$(ls -td "$DIR_PROGRAMAS"/*/ | head -n 1)
...
if [[ -n "$latestDirectory" ]]; then
    ...
    if [[ ${#xmlFiles[@]} -gt 0 ]]; then
        for xmlFile in "${xmlFiles[@]}"; do
            ...
            sql_command="EXEC dbo.SP_INSERT_XML_PROGRAMAS '${latestDirName}', '${xmlContentEscaped}'"

            # Construct SQL connection string and execute SQL command using sqlcmd
            sqlcmd -S "${DB_HOST},${DB_PORT}" -d PROGRAMAS -U "${DB_USER}" -P "${DB_PASSWORD}" -Q "${sql_command}"
        done
    ...
    echo "No directories found in $DIR_PROGRAMAS."
fi
